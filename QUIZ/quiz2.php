...
<head>
  ...
  <title>RUPANTARA - Quiz Level 2</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: "Plus Jakarta Sans", "Inter", sans-serif; }
    ...
    .question-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #7B4AC9, #A855F7); border-radius: var(--radius-lg) var(--radius-lg) 0 0; transform: scaleX(0); transform-origin: left; transition: transform 0.5s ease; }
    ...
    .question-number { font-size: 12px; font-weight: 700; color: #7B4AC9; background: #F3E8FF; padding: 4px 12px; border-radius: 999px; display: inline-block; width: fit-content; animation: fadeIn 0.4s ease; }
    .level-badge { font-size: 12px; font-weight: 700; color: #5B3A94; background: linear-gradient(135deg, #E7DEF7, #D6C4F0); padding: 4px 12px; border-radius: 999px; display: inline-flex; align-items: center; gap: 5px; width: fit-content; animation: fadeIn 0.4s ease .05s both; box-shadow: 0 2px 8px rgba(123,74,201,0.2); }
    .level-badge .level-dot { width: 6px; height: 6px; border-radius: 50%; background: #7B4AC9; }
    ...
    .option:hover { border-color: #7B4AC9; background: #FBF8FF; transform: translateY(-2px) scale(1.01); box-shadow: 0 6px 20px rgba(123,74,201,0.1); }
    .option:active { transform: translateY(0) scale(0.98); }
    .option.selected { border-color: #7B4AC9; background: linear-gradient(135deg, #F3E8FF, #E9D5FF); box-shadow: 0 4px 15px rgba(123,74,201,0.15); transform: translateY(-1px); }
    ...
    .option.selected .option-badge { background: #7B4AC9; color: #fff; transform: scale(1.1); }
    ...
    .result-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 5px; background: linear-gradient(90deg, #7B4AC9, #A855F7); }
    .result-badge { align-self: center; font-size: 12px; font-weight: 700; padding: 8px 18px; border-radius: 999px; background: #F3E8FF; color: #7B4AC9; animation: fadeIn 0.5s ease 0.2s both; }
...
<!-- HEADER (Floating Glassmorphism style - sama persis dengan edukasi.php & quiz1.php) -->
<nav>
        ...
        <div class="question-card" id="questionCard">
          <div class="question-head">
            <div class="question-badges">
              ...
              <div class="level-badge"><span class="level-dot"></span> Level 2 &middot; Menengah</div>
            </div>
...
<div class="result-overlay" id="resultOverlay">
  <div class="result-card" id="resultCard">
    ...
    <div class="level-prompt" id="levelPrompt">Mantap, Level 2 berhasil kamu selesaikan! Mau ulangi biar sempurna, atau kembali ke Beranda?</div>
    <div class="result-actions">
      <a href="quiz2.php" class="btn btn-secondary" id="retryBtn" style="text-decoration:none;">&#8635; Ulangi Level 2</a>
      <a href="../BERANDA/beranda.php" class="btn btn-success" id="nextLevelBtn" style="text-decoration:none;">Kembali ke Beranda &#8594;</a>
    </div>
...
<script>
// Data 10 Soal Kuis Level 2 (Berdasarkan Materi di edukasi.php - Tingkat Mudah)
const SOAL = [
  { 
    pertanyaan: "Logo Bank Indonesia pada perisai yang berubah warna jika dilihat dari sudut pandang berbeda menggunakan teknologi...",
    opsi: [
      {l:'A', t:'Tinta Biasa'},
      {l:'B', t:'Optically Variable Ink (OVI)'},
      {l:'C', t:'Tinta Stempel'},
      {l:'D', t:'Spidol Warna'}
    ], 
    benar:'B',
    pembahasan: "Optically Variable Ink (OVI) membuat logo BI berubah warna saat dilihat dari sudut pandang berbeda." 
  },
  { 
    pertanyaan: "Tujuan utama dan tunggal Bank Indonesia menurut undang-undang adalah...",
    opsi: [
      {l:'A', t:'Mencetak uang sebanyak-banyaknya'},
      {l:'B', t:'Mencapai dan memelihara kestabilan nilai Rupiah'},
      {l:'C', t:'Membagikan uang tunai kepada masyarakat'},
      {l:'D', t:'Menjadi bank penyimpan tabungan swasta'}
    ], 
    benar:'B',
    pembahasan: "Tujuan tunggal Bank Indonesia adalah mencapai dan memelihara kestabilan nilai Rupiah." 
  },
  { 
    pertanyaan: "Salah satu uang kuno logam perunggu peninggalan Kerajaan Majapahit yang terkenal adalah...",
    opsi: [
      {l:'A', t:'Uang Gobog'},
      {l:'B', t:'Uang Kertas ORI'},
      {l:'C', t:'Gulden Belanda'},
      {l:'D', t:'Uang Koin Dolar'}
    ], 
    benar:'A',
    pembahasan: "Uang Gobog merupakan uang koin logam kuno bersejarah peninggalan era Kerajaan Majapahit." 
  },
  { 
    pertanyaan: "Di bawah ini yang BUKAN merupakan larangan dalam prinsip 5J merawat Rupiah adalah...",
    opsi: [
      {l:'A', t:'Jangan Dicoret'},
      {l:'B', t:'Jangan Dibelanjakan'},
      {l:'C', t:'Jangan Diremas'},
      {l:'D', t:'Jangan Dibasahi'}
    ], 
    benar:'B',
    pembahasan: "Prinsip 5J adalah Jangan Dilipat, Jangan Dicoret, Jangan Distaples, Jangan Diremas, dan Jangan Dibasahi. Uang tentu boleh dibelanjakan." 
  },
  { 
    pertanyaan: "Berbeda dengan kripto yang harganya naik turun, 1 Rupiah Digital dijamin bernilai...",
    opsi: [
      {l:'A', t:'Berubah-ubah setiap menit'},
      {l:'B', t:'Pasti bernilai 1:1 dengan Rupiah fisik'},
      {l:'C', t:'Mengikuti harga koin emas'},
      {l:'D', t:'Dua kali lipat uang kertas'}
    ], 
    benar:'B',
    pembahasan: "Rupiah Digital diterbitkan resmi oleh Bank Indonesia sehingga nilainya selalu bernilai pasti 1:1 dengan Rupiah fisik." 
  },
  { 
    pertanyaan: "Sebelum dinasionalisasi menjadi Bank Indonesia pada tahun 1953, bank sentral era Hindia Belanda bernama...",
    opsi: [
      {l:'A', t:'Bank Rakyat Indonesia'},
      {l:'B', t:'De Javasche Bank'},
      {l:'C', t:'Bank Mandiri'},
      {l:'D', t:'Bank Negara Indonesia'}
    ], 
    benar:'B',
    pembahasan: "De Javasche Bank resmi dinasionalisasi pemerintah Republik Indonesia pada tahun 1953 menjadi Bank Indonesia (BI)." 
  },
  { 
    pertanyaan: "Tanda air (Watermark) gambar pahlawan pada uang kertas Rupiah asli akan terlihat saat...",
    opsi: [
      {l:'A', t:'Diterawang ke arah cahaya'},
      {l:'B', t:'Dicuci memakai air hangat'},
      {l:'C', t:'Digosok menggunakan penghapus'},
      {l:'D', t:'Disimpan di dalam freezer'}
    ], 
    benar:'A',
    pembahasan: "Watermark atau tanda air tertanam di serat kertas dan akan tampak jelas saat uang diterawang ke arah cahaya." 
  },
  { 
    pertanyaan: "Proyek resmi Bank Indonesia dalam merancang dan mengembangkan Rupiah Digital disebut...",
    opsi: [
      {l:'A', t:'Proyek Merah Putih'},
      {l:'B', t:'Proyek Garuda'},
      {l:'C', t:'Proyek Cendrawasih'},
      {l:'D', t:'Proyek Satria'}
    ], 
    benar:'B',
    pembahasan: "Proyek Garuda adalah inisiatif resmi Bank Indonesia dalam memayungi eksplorasi desain arsitektur Rupiah Digital (CBDC)." 
  },
  { 
    pertanyaan: "Mengapa kita sangat dianjurkan menggunakan dompet panjang saat membawa uang kertas Rupiah?",
    opsi: [
      {l:'A', t:'Supaya uang tidak terlipat dan seratnya tetap awet'},
      {l:'B', t:'Agar dompet terlihat lebih tebal'},
      {l:'C', t:'Supaya uang bisa beranak pinak'},
      {l:'D', t:'Menghindari uang hilang di pasar'}
    ], 
    benar:'A',
    pembahasan: "Dompet panjang menjaga uang tetap lurus, tidak tertekuk, serta mencegah patahnya serat kertas uang." 
  },
  { 
    pertanyaan: "Suku bunga acuan yang ditetapkan Bank Indonesia untuk mengendalikan kestabilan inflasi disebut...",
    opsi: [
      {l:'A', t:'Pajak Penjualan'},
      {l:'B', t:'BI-Rate'},
      {l:'C', t:'Kurs Dolar'},
      {l:'D', t:'Tabungan Deposito'}
    ], 
    benar:'B',
    pembahasan: "BI-Rate (suku bunga acuan) adalah instrumen utama kebijakan moneter BI untuk mengendalikan inflasi dan stabilitas moneter." 
  }
];
...
function launchConfetti(){
  ...
  const colors=['#7B4AC9','#17325C','#1E8E5A','#E8A838','#D0392B','#59A9E8'];
  const particles=[];
  for(let i=0;i<150;i++){
    particles.push({
...
function saveProgress(){
  ...
  localStorage.setItem('rupantara_quiz_level2_progress',JSON.stringify(data));
  el.saveIndicator.classList.add('show'); setTimeout(()=>el.saveIndicator.classList.remove('show'),2000);
}

function loadProgress(){
  try{
    const saved=localStorage.getItem('rupantara_quiz_level2_progress'); if(!saved) return false;
    const data=JSON.parse(saved);
    if(Date.now()-data.timestamp>24*60*60*1000){ localStorage.removeItem('rupantara_quiz_level2_progress'); return false; }
...
function clearProgress(){ localStorage.removeItem('rupantara_quiz_level2_progress'); }
...
function selesaiQuiz(kehabisanWaktu){
  ...
  if(kehabisanWaktu){
    ...
    el.resultCard.classList.remove('gagal'); el.resultBadge.textContent='Hasil Level 2';
    playSound('finish');
    if(persen>=80) launchConfetti();
    el.resultScore.textContent=benar+' / '+SOAL.length;
    if(persen===100) el.resultMsg.textContent='Sempurna! 🌟 Kamu paham banget materi Rupiah!';
  ...
  if(kehabisanWaktu){
    el.levelPrompt.textContent='Waktu habis nih. Yuk ulangi Level 2, atau kembali dulu ke Beranda.';
  } else if(persen>=60){
    el.levelPrompt.textContent='Mantap, Level 2 berhasil kamu selesaikan! Mau ulangi biar sempurna, atau kembali ke Beranda?';
  } else {
    el.levelPrompt.textContent='Skor kamu masih bisa ditingkatkan. Yuk ulangi Level 2 dulu, atau kembali ke Beranda.';
  }
...
el.backBtn.addEventListener('click',()=>{ window.location.href = 'quiz_intro.php'; });
...
mulai();
if(loaded) showToast('Progress tersimpan ditemukan! Melanjutkan quiz Level 2...', 'info', 4000);
lucide.createIcons();
</script>
</body>
</html>